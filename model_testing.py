...
# Save raw results
with open("model_temp_test.json", "w") as f:
...
print(f"\nResults saved to:")
print(f"- Raw data: model_temp_test.json")
print(f"- Comparison CSV: model_comparison.csv")
print(f"CSV has {len(comparison_df)} rows with columns: {list(comparison_df.columns)}")